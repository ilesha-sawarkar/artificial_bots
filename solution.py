...
#		print(self.fitness)
	...
	def Mutate(self):
		...
		add_remove_none = 2
		if ((len(list(child_links)) > 3 and len(list(child_links)) < c.maxLinks) and self.sensorNeuronList.count(1)>=2):
			add_remove_none = random.choice([0, 1, 2])
			# add_remove_none = 0
		
		elif (len(list(child_links)) > 3 and len(list(child_links)) >= c.maxLinks and self.sensorNeuronList.count(1)>=2):
			add_remove_none = random.choice([0, 2])
		elif  (len(list(child_links)) < 3 and self.sensorNeuronList.count(1)<2):
...
#		self.weights[randomRow,randomColumn] =  random.random() * 2 - 1
	...
	def Create_Body(self):
		...
		for i in range (0, self.number_of_links):
			...
			if(i == 0):
				shapeInfo["Link" + str(i)] = [[length, width, height],[minX,maxX],[minY,maxY],[minZ,maxZ]]
				shapesAdded.append("Link" + str(i))
				...
				while(flag2 == 1):
					...
					if ([jointPosition_Direction,linkToJoin] in connections):
						pass
					else:
						linkToJoinPointX = shapeInfo[linkToJoin][1]
						linkToJoinPointY = shapeInfo[linkToJoin][2]
						linkToJoinPointZ = shapeInfo[linkToJoin][3]
				...
				shapesAdded.append("Link" + str(i))
				connections.append([jointPosition_Direction,linkToJoin])  
				total_creature_connections[linkToJoin+"_"+"Link"+ str(i)] = jointPosition_Direction
				self.connections = connections
				
				for li in reversed(range(len(self.LinkJoints))) :
					if ("_"+linkToJoin) in self.LinkJoints[li]:
						grandparentLink = self.LinkJoints[li]
				...
				if (linkToJoin == "Link0"):
					if (jointPosition_Direction == 0):
						pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [shapeInfo[linkToJoin][0][0], shapeInfo[linkToJoin][0][1]/2, shapeInfo[linkToJoin][0][2]/2], jointAxis = "1 0 0")
					elif (jointPosition_Direction == 1):
						pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [shapeInfo[linkToJoin][0][0]/2, shapeInfo[linkToJoin][0][1], shapeInfo[linkToJoin][0][2]/2], jointAxis = "0 1 0")
					elif (jointPosition_Direction == 2):
						pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [shapeInfo[linkToJoin][0][0]/2, shapeInfo[linkToJoin][0][1]/2, shapeInfo[linkToJoin][0][2]], jointAxis = "0 0 1")
					elif (jointPosition_Direction == 3):
						pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [0, shapeInfo[linkToJoin][0][1]/2, shapeInfo[linkToJoin][0][2]/2], jointAxis = "1 0 0")
					elif (jointPosition_Direction == 4):
						pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [shapeInfo[linkToJoin][0][0]/2, 0, shapeInfo[linkToJoin][0][2]/2], jointAxis = "0 1 0")
					else:
						pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [shapeInfo[linkToJoin][0][0]/2, shapeInfo[linkToJoin][0][1]/2, 0], jointAxis = "0 1 0")
					self.motorNeuronList.append(linkToJoin+"_"+"Link" + str(i))


				elif(grandParAxis == jointPosition_Direction):
					if (jointPosition_Direction == 0):
						pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [shapeInfo[linkToJoin][0][0],0,0], jointAxis = "1 0 0")
					elif (jointPosition_Direction == 1):
						pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [0,shapeInfo[linkToJoin][0][1],0], jointAxis = "0 1 0")
					elif (jointPosition_Direction == 2):
						pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin  , child = "Link" + str(i) , type = "revolute", position = [0,0,shapeInfo[linkToJoin][0][2]], jointAxis = "0 0 1")
					elif (jointPosition_Direction == 3):
						pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [-(shapeInfo[linkToJoin][0][0]),0,0], jointAxis = "1 0 0")
					elif (jointPosition_Direction == 4):
						pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [0,-(shapeInfo[linkToJoin][0][1]),0], jointAxis = "0 1 0")
					...
					if (grandParAxis == 0):
						if (jointPosition_Direction == 1):
							...
							pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin  , child = "Link" + str(i) , type = "revolute", position = [shapeInfo[linkToJoin][0][0]/2, 0, shapeInfo[linkToJoin][0][2]/2], jointAxis = "0 0 1")
						elif (jointPosition_Direction == 4):
							pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [shapeInfo[linkToJoin][0][0]/2, -(shapeInfo[linkToJoin][0][1]/2), 0], jointAxis = "0 1 0")
						...
						if (jointPosition_Direction == 0):
							pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [shapeInfo[linkToJoin][0][0]/2, shapeInfo[linkToJoin][0][1]/2, 0], jointAxis = "1 0 0")
						elif (jointPosition_Direction == 2):
							pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin  , child = "Link" + str(i) , type = "revolute", position = [0, shapeInfo[linkToJoin][0][1]/2, shapeInfo[linkToJoin][0][2]/2], jointAxis = "0 0 1")
						elif (jointPosition_Direction == 3):
							pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [-(shapeInfo[linkToJoin][0][0]/2), shapeInfo[linkToJoin][0][1]/2, 0], jointAxis = "1 0 0")
						elif (jointPosition_Direction == 5):
							pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin  , child = "Link" + str(i) , type = "revolute", position = [0, shapeInfo[linkToJoin][0][1]/2, -(shapeInfo[linkToJoin][0][2]/2)], jointAxis = "0 0 1")
						...
						if (jointPosition_Direction == 0):
							pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [shapeInfo[linkToJoin][0][0]/2, 0,  shapeInfo[linkToJoin][0][2]/2], jointAxis = "1 0 0")
						elif (jointPosition_Direction == 1):
							pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin  , child = "Link" + str(i) , type = "revolute", position = [0, shapeInfo[linkToJoin][0][1]/2, shapeInfo[linkToJoin][0][2]/2], jointAxis = "0 1 0")
						elif (jointPosition_Direction == 3):
							pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin , child = "Link" + str(i) , type = "revolute", position = [-(shapeInfo[linkToJoin][0][0]/2), 0,  shapeInfo[linkToJoin][0][2]/2], jointAxis = "1 0 0")
						elif (jointPosition_Direction == 4):
							pyrosim.Send_Joint(name = linkToJoin + "_" + "Link" + str(i) , parent = linkToJoin  , child = "Link" + str(i) , type = "revolute", position = [0, -(shapeInfo[linkToJoin][0][1]/2), shapeInfo[linkToJoin][0][2]/2], jointAxis = "0 1 0")
				...
				if (jointPosition_Direction == 0):
					...
					self.Send_Shape(shape_choice, name = "Link" + str(i), pos=[0,0,-(height/2)], size=[length, width, height],mass=1.0, material_name = color_name, rgba=rgba_string )	
				self.LinkJoints.append(linkToJoin + "_" + "Link" + str(i))
		self.shapeInfo = shapeInfo
		...
		for link in shapesAdded:
			...
			if (link == "Link0"):
				...
				if (JoiningLink == "Link0"):
					if (jointPosition_Direction == 0):
						pyrosim.Send_Joint(name = JoiningLink + "_" + link , parent = JoiningLink , child = link , type = "revolute", position = [shapeInfo[JoiningLink][0][0], shapeInfo[JoiningLink][0][1]/2, shapeInfo[JoiningLink][0][2]/2], jointAxis = "1 0 0")
					elif (jointPosition_Direction == 1):
						pyrosim.Send_Joint(name = JoiningLink + "_" + link , parent = JoiningLink , child = link , type = "revolute", position = [shapeInfo[JoiningLink][0][0]/2, shapeInfo[JoiningLink][0][1], shapeInfo[JoiningLink][0][2]/2], jointAxis = "0 1 0")
					elif (jointPosition_Direction == 2):
						...
						pyrosim.Send_Joint(name = JoiningLink + "_" + link , parent = JoiningLink , child = link , type = "revolute", position = [0, shapeInfo[JoiningLink][0][1]/2, shapeInfo[JoiningLink][0][2]/2], jointAxis = "1 0 0")
					elif (jointPosition_Direction == 4):
						pyrosim.Send_Joint(name = JoiningLink + "_" + link , parent = JoiningLink , child = link , type = "revolute", position = [shapeInfo[JoiningLink][0][0]/2, 0, shapeInfo[JoiningLink][0][2]/2], jointAxis = "0 1 0")
					else:                        
					...
					self.motorNeuronList.append(JoiningLink+"_"+"Link" + link)
